...
import { ArrowLeftRight, ChevronRight, Globe2, Headphones, Mic, SmartphoneNfc, Volume2 } from "lucide-react";
...
const STEPS = [
  {
    ...
    desc: "Configure Language A and Language B. VoiceLink auto-detects which one you're speaking.",
  },
  {
    icon: Mic,
    step: "02",
...
function WaveformDemo({ active }: { active: boolean }) {
  const heights = [0.3, 0.6, 0.9, 0.5, 1, 0.7, 0.4, 0.85, 0.55, 0.7, 0.4, 0.9, 0.6];
  return (
      ...
      {heights.map((h, i) => (
        <div
          ...
          style={{
            ...
            background: active ? `rgba(255,255,255,${0.35 + h * 0.55})` : "rgba(255,255,255,0.12)",
      ...
      >
        ...
        <div className="flex items-stretch gap-2 px-3 pb-2 pt-1">
          ...
          <div className="flex-1 rounded-xl bg-white/[0.04] border border-white/[0.07] p-2.5">
            <div className="text-[9px] text-white/20 uppercase tracking-widest mb-1.5">Person A</div>
            <div className="flex items-center gap-1.5 mb-2">
              <span className="text-lg leading-none">{conv.fromFlag}</span>
              <span className="text-xs font-semibold text-white/80 truncate">{conv.fromLang}</span>
            </div>
          ...
          {/* Card B */}
          <div className="flex-1 rounded-xl bg-white/[0.04] border border-white/[0.07] p-2.5">
            <div className="text-[9px] text-white/20 uppercase tracking-widest mb-1.5">Person B</div>
            <div className="flex items-center gap-1.5 mb-2">
              <span className="text-lg leading-none">{conv.toFlag}</span>
              <span className="text-xs font-semibold text-white/80 truncate">{conv.toLang}</span>
            </div>
            <div className="h-px bg-white/[0.06] mb-2" />
            <div className="flex items-center gap-1 text-[9px] text-white/25">
              <Mic className="w-2.5 h-2.5" /> Default mic
        ...
        <div className="h-[110px] px-3 py-2 flex flex-col justify-end overflow-hidden">
          ...
          {phase === "idle" && (
            <div className="flex flex-col items-center justify-center h-full gap-2">
              ...
              <p className="text-[9px] text-white/15 text-center">Hold to speak in {conv.fromLang} or {conv.toLang}</p>
            </div>
          )}
          ...
          {isRecording && (
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[9px] text-white/30 uppercase tracking-widest">{conv.fromLang}</span>
              </div>
          ...
          {isResult && (
            <div
              className="flex flex-col gap-1 animate-in fade-in slide-in-from-bottom-2 duration-500"
            >
              <div className="flex items-center gap-1">
                <span className="text-sm">{conv.fromFlag}</span>
                <span className="text-[9px] text-white/30 uppercase tracking-widest">{conv.fromLang}</span>
              </div>
              <p className="text-[10px] text-white/40 leading-snug pl-5">{conv.sourceText}</p>
              <div className="flex items-start gap-1 mt-1">
                <span className="text-sm">{conv.toFlag}</span>
                <p className="text-[11px] text-white font-medium leading-snug">{conv.translatedText}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col items-center gap-2 py-4">
          ...
          >
            ...
            <Mic className={`w-5 h-5 ${isRecording ? "text-white" : "text-zinc-900"}`} />
          </div>
        </div>
      </div>
    ...
    <div className="w-full min-h-[100dvh] bg-matte-dark text-white relative overflow-y-auto overflow-x-hidden grain">
      ...
      <header className="relative z-20 flex items-center justify-between px-6 md:px-14 py-6 border-b border-white/[0.05]">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-white/60 text-sm font-semibold tracking-wide">VoiceLink</span>
        </div>
        <button
        ...
        <div className="flex flex-col md:flex-row items-center gap-12 md:gap-16">
          ...
          <div className="flex-1 flex flex-col items-start text-left max-w-xl">
            ...
            <p className="text-white/40 text-base md:text-lg leading-relaxed mb-10 max-w-md">
              VoiceLink lets two people speak face-to-face in completely different languages — and actually understand each other — in real time.
            </p>
            ...
            <p className="mt-3 text-white/15 text-xs">Works instantly in your browser. No install.</p>
          </div>
      ...
      <section className="relative z-10 px-6 py-20 border-t border-white/[0.05]">
        <div className="max-w-4xl mx-auto">
          <p className="text-white/25 text-xs uppercase tracking-widest mb-4 text-center">The problem we solve</p>
          <h2 className="font-display text-3xl md:text-5xl font-bold text-center mb-5 leading-tight">
            You're standing in front of someone.<br />
            <span className="text-white/30">Neither of you can say a word.</span>
          </h2>
          <p className="text-center text-white/35 text-base md:text-lg max-w-2xl mx-auto mb-14 leading-relaxed">
            You're a traveller. You meet locals, shopkeepers, hotel staff, strangers who want to help — but the conversation stops before it starts. Translation apps require you to type, wait, and show a screen. That's not a conversation.
          </p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {PROBLEMS.map(({ emoji, scene }) => (
              <div key={scene} className="flex flex-col items-center gap-3 p-5 rounded-2xl bg-white/[0.03] border border-white/[0.07] text-center hover:bg-white/[0.05] transition-colors">
      ...
      <section className="relative z-10 px-6 py-20 border-t border-white/[0.05]">
        <div className="max-w-4xl mx-auto">
          <p className="text-white/25 text-xs uppercase tracking-widest mb-4 text-center">How it works</p>
          <h2 className="font-display text-3xl md:text-4xl font-bold text-center mb-14">Three steps to a full conversation.</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {STEPS.map(({ icon: Icon, step, title, desc }) => (
              <div key={step} className="flex flex-col gap-4 p-6 rounded-2xl bg-white/[0.03] border border-white/[0.07] hover:bg-white/[0.05] transition-colors">
                <div className="flex items-center justify-between">
                  <div className="w-9 h-9 rounded-xl bg-white/[0.06] border border-white/10 flex items-center justify-center">
                    <Icon className="w-4 h-4 text-white/60" />
                  </div>
                  <span className="font-display text-4xl font-bold text-white/[0.06]">{step}</span>
                </div>
                <h3 className="font-semibold text-white/80 text-base leading-snug">{title}</h3>
      ...
      <section className="relative z-10 px-6 py-24 border-t border-white/[0.05]">
        <div className="max-w-2xl mx-auto text-center flex flex-col items-center gap-6">
          <h2 className="font-display text-3xl md:text-5xl font-bold leading-tight">
            Your next trip, without<br />
            <span className="text-white/30">language anxiety.</span>
          </h2>
          <p className="text-white/35 text-base max-w-md leading-relaxed">
            Open VoiceLink, pick your languages, and have a real conversation — wherever you are in the world.
          </p>
          <button
            onClick={() => setLocation("/translate")}
            className="group inline-flex items-center gap-3 px-8 py-4 rounded-full bg-white text-zinc-950 font-semibold text-[15px] hover:bg-white/90 transition-all duration-200 active:scale-95"
          >
            <Mic className="w-4 h-4" />
            Start Translating — Free
      ...
      <footer className="relative z-10 px-6 md:px-14 py-6 border-t border-white/[0.05] flex items-center justify-between">
        <span className="text-white/15 text-xs">Powered by OpenAI · Lingo.dev</span>
        <span className="text-white/15 text-xs">VoiceLink © 2026</span>
      </footer>
    </div>
  );
}
